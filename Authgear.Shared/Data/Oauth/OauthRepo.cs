...
namespace Authgear.Xamarin.Data.Oauth
{
    ...
    internal interface OauthRepoDelegate
    {
        void ClearSession(SessionStateChangeReason reason);
    }
    ...
    internal class OauthRepo : IOauthRepo
    {
        ...
        internal OauthRepoDelegate? sdk;
        ...
        private void handleException(Exception ex)
        {
            while (ex != null)
            {
                if (ex is OauthException)
                {
                    ...
                    {
                        this.sdk?.ClearSession(SessionStateChangeReason.Invalid);
                        return;
                    }
                }
        ...
        public async Task<AppSessionTokenResponse> OauthAppSessionTokenAsync(string refreshToken)
        {
            try
            {
                return await this.impl.OauthAppSessionTokenAsync(refreshToken);
            } catch (Exception ex)
            {
                this.handleException(ex);
                throw;
        ...
        public async Task<ChallengeResponse> OauthChallengeAsync(string purpose)
        {
            try
            {
                return await this.impl.OauthChallengeAsync(purpose);
            } catch (Exception ex)
            {
                this.handleException(ex);
                throw;
        ...
        public async Task<OidcTokenResponse> OidcTokenRequestAsync(OidcTokenRequest request)
        {
            try
            {
                return await this.impl.OidcTokenRequestAsync(request);
            } catch (Exception ex)
            {
                this.handleException(ex);
                throw;
